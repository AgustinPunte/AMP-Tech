...
float get_location(int x){
  ...
  else{                         // Al no haber coneccion se envia una posicion dentro de la granja
    if (x==0){
      return 41.97864;
      }
      if (x==1){
...
float get_level(){
  ...
  if (level==0){                         // Para la prueba de concepto si el sensor no funciona envia 50. 
    return level=50;                     // en la aplicacion real sera un numero fuera de rango
    }
  else{
    return round(level);
    }
...
void publishSmallJson() {
  ...
  doc["lat"]  = get_location(0);
  doc["lon"]  = get_location(1);
  doc["lv"]   = get_level();
